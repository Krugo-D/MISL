...
contract MISLV24 is Context, IERC20, Ownable {
    ...
    string private _name = "MISSLEv24";
    string private _symbol = "MISLV24";
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapMarketingAndSendEth(contractTokenBalance); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        if(marketingTransferEnabled) {
            payable(marketingAndTeamFeeWallet).transfer(address(this).balance);
        }
    ...
    function setMarketingTransfer(bool _enabled) public onlyOwner {
        marketingTransferEnabled = _enabled;
        //emit SwapAndLiquifyEnabledUpdated(_enabled);
    }
