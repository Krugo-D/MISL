...
contract MISLV25 is Context, IERC20, Ownable {
    ...
    struct RValuesStruct {
        ...
        uint256 rFee;
        uint256 rBurn;
        uint256 rMarketingToken;
        uint256 rMarketingEth;
    }
    ...
    struct TValuesStruct {
        ...
        uint256 tFee;
        uint256 tBurn;
        uint256 tMarketingToken;
        uint256 tMarketingEth;
    }
    ...
    string private _name = "MISSLEv25";
    string private _symbol = "MISLV25";
    ...
    address public marketingWallet = 0x8cd48A7F0f72DF02f0D308Fe270487DE353177A1;

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;

    bool inMarketingEthSwap = false;
    bool public marketingEthSwapEnabled = false;
    bool public tradingEnabled = false;

    uint256 public _maxTxAmount = 1000000000 * 10**9;
    uint256 private numTokensSellToSwapToMarketingEth = 100000 * 10**9;
    ...
    function _distributeFee(uint256 rFee, uint256 rBurn, uint256 rMarketingToken, uint256 tFee, uint256 tBurn, uint256 tMarketingToken) private {
        _rTotal = _rTotal.sub(rFee).sub(rBurn);
        _tFeeTotal = _tFeeTotal.add(tFee);
        _tTotal = _tTotal.sub(tBurn);
        _tBurnTotal = _tBurnTotal.add(tBurn);

        _rOwned[marketingWallet] = _rOwned[marketingWallet].add(rMarketingToken);
        if (_isExcluded[marketingWallet]) {
            _tOwned[marketingWallet] = _tOwned[marketingWallet].add(tMarketingToken);
        }
    }

    function _getValues(uint256 tAmount) private view returns (ValuesStruct memory) {
        TValuesStruct memory tvs = _getTValues(tAmount);
        RValuesStruct memory rvs = _getRValues(tAmount, tvs.tFee, tvs.tBurn, tvs.tMarketingToken, tvs.tMarketingEth, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
        ...
        uint256 tMarketingToken = calculateMarketingTokenFee(tAmount);
        uint256 tMarketingEth = calculateMarketingEthFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurn).sub(tMarketingToken).sub(tMarketingEth);
        return TValuesStruct(tTransferAmount, tFee, tBurn, tMarketingToken, tMarketingEth);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tMarketingToken, uint256 tMarketingEth, uint256 currentRate) private pure returns (RValuesStruct memory) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rMarketingToken = tMarketingToken.mul(currentRate);
        uint256 rMarketingEth = tMarketingEth.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMarketingEth).sub(rBurn).sub(rMarketingToken);
        return RValuesStruct(rAmount, rTransferAmount, rFee, rBurn, rMarketingToken, rMarketingEth);
    }
    ...
    function _takeMarketingEth(uint256 rMarketingEth, uint256 tMarketingEth) private {
        _rOwned[address(this)] = _rOwned[address(this)].add(rMarketingEth);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tMarketingEth);
    }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= numTokensSellToSwapToMarketingEth;
        if (
            overMinTokenBalance &&
            !inMarketingEthSwap &&
            from != uniswapV2Pair &&
            marketingEthSwapEnabled
        ) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        _takeMarketingEth(vs.rMarketingEth, vs.tMarketingEth);
        _distributeFee(vs.rFee, vs.rBurn, vs.rMarketingToken, vs.tFee, vs.tBurn, vs.tMarketingToken);
    ...
    function setSwapAndPayMarketingEthEnabled(bool _enabled) public onlyOwner {
        marketingEthSwapEnabled = _enabled;
    }
    ...
    function setMarketingEthSwap(bool _enabled) public onlyOwner {
        marketingEthSwapEnabled = _enabled;
    }

   // Number of Tokens to Accrue before swapping to ETH and pay to marketingWallet
	function setnumTokensSellToSwapToMarketingEth(uint256 tokenAmount) external onlyOwner() {
       numTokensSellToSwapToMarketingEth = tokenAmount;
    }
